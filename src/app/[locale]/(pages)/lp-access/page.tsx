"use client"
import fetchData, { LogIn } from "@/utils/api"
...
export default function LpAccess() {
	...
	useEffect(() => {
		async function getOptions() {
			const _options = await fetchData({ route: "option" })
			console.log("options: ", _options?.data)
				...
				>
					...
					<div className="flex items-center justify-start gap-2">
						<input
							type="checkbox"
							id="remember_me"
							checked={values.remreberMe}
							onChange={(e) => setValues({ ...values, remreberMe: e.target.checked })}
						/>
