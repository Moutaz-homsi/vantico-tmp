...
export default function ROICalculator() {
  const [monthlyDeposit, setMonthlyDeposit] = useState(4000)
  const [years, setYears] = useState("15")
  const [yield_, setYield] = useState("6")
  const [chartData, setChartData] = useState<any[]>([])
  const [portfolioValue, setPortfolioValue] = useState(0)
  const [monthlyRental, setMonthlyRental] = useState(0)

  useEffect(() => {
    // Calculate portfolio value based on monthly deposits, years, and yield
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm">
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-medium">Monthly Deposits</h3>
              <p className="text-2xl font-bold">AED {formatCurrency(monthlyDeposit)}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-emerald-50 p-4 rounded-lg">
                <h3 className="text-2xl font-bold text-emerald-700">AED {formatCurrency(portfolioValue)}</h3>
                <p className="text-sm text-emerald-700">Projected Portfolio value by year {years}</p>
              </div>
              <div className="bg-emerald-50 p-4 rounded-lg">
                <h3 className="text-2xl font-bold text-emerald-700">AED {formatCurrency(monthlyRental)}</h3>
